...
class SemesterSchedule(BaseModel):
    ...
    courses: List[CourseInSchedule] = Field(default_factory=list, description="Courses in this semester")
...
class OptimizedScheduleResponse(BaseModel):
    ...
    schedule: List[SemesterSchedule] = Field(..., description="Optimized semester-by-semester schedule")
    ...
    warnings: List[str] = Field(default_factory=list, description="Any scheduling warnings or conflicts")
